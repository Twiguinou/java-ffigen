...
public sealed interface CXUnsavedFile permits CXUnsavedFile.Windows, CXUnsavedFile.Standard
{
    ...
    default MemorySegment Filename() {return this.ptr().get(UNBOUNDED_POINTER, OFFSET__Filename);}
    default void Filename(MemorySegment value) {this.ptr().set(UNBOUNDED_POINTER, OFFSET__Filename, value);}
    default MemorySegment $Filename() {return this.ptr().asSlice(OFFSET__Filename, UNBOUNDED_POINTER);}

    default MemorySegment Contents() {return this.ptr().get(UNBOUNDED_POINTER, OFFSET__Contents);}
    ...
    record Windows(MemorySegment ptr) implements CXUnsavedFile
    {
        ...
        public MemorySegment $Length() {return this.ptr().asSlice(OFFSET__Length, JAVA_INT);}
    }

    record Standard(MemorySegment ptr) implements CXUnsavedFile
    {
        public long Length() {return this.ptr().get(JAVA_LONG, OFFSET__Length);}
        public void Length(long value) {this.ptr().set(JAVA_LONG, OFFSET__Length, value);}
        public MemorySegment $Length() {return this.ptr().asSlice(OFFSET__Length, JAVA_LONG);}
    }
}
