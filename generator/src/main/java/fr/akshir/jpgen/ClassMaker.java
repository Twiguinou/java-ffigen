...
{private ClassMaker() {}
    ...
    public static void makeHeader(PrintingContext context, HeaderDeclaration header) throws IOException
    {
        ...
        context.breakLine('{');
        context.pushControlFlow();
        context.breakLine("java.lang.foreign.SymbolLookup lookup = name -> java.lang.foreign.SymbolLookup.loaderLookup().find(name).or(() -> fr.akshir.jpgen.NativeTypes.SYSTEM_LINKER.defaultLookup().find(name));");
        ...
        for (Binding binding : header.bindings())
        {
            if (binding.getFunctionHandle().isEmpty())
            {
                ...
                context.append("MTD__").append(name).append(" = fr.akshir.jpgen.NativeTypes.SYSTEM_LINKER.downcallHandle(MTD_ADDRESS__").append(name).append(", ").append(getFunctionDescriptor(binding.function.descriptorType)).breakLine(
    ...
    public static void makeCallback(PrintingContext context, CallbackDeclaration callback) throws IOException
    {
        ...
        context.append("java.lang.invoke.MethodHandle ").append(callback.stubName).append(" = fr.akshir.jpgen.NativeTypes.initUpcallStub(").append(callback.descriptorName).append(", \"");
        ...
        context.breakLine('{');
        context.pushControlFlow();
        context.append("return fr.akshir.jpgen.NativeTypes.SYSTEM_LINKER.upcallStub(").append(callback.stubName).append(".bindTo(this), ").append(callback.descriptorName).breakLine(", arena);");
        context.popControlFlow();
        context.breakLine('}');
